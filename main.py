...
def check_gpu():
    ...
    if gpus:
        try:
            ...
            print(
                f"INFO: GPU found and configured: {gpus[0].name}. Proceeding with GPU acceleration."
            )
...
def main():
    ...
    if model is None:
        ...
        user_input = (
            input("INFO: Model found. Do you want to retrain it? (yes/no): ")
            .lower()
            .strip()
        )
        ...
        if model_output_units is not None and getattr(y, "ndim", 1) > 1:
            ...
            if model_output_units != current_units:
                ...
                if model_output_units > current_units:
                    ...
                    print(
                        f"INFO: Padded label one-hot vectors with {pad_width} zero columns to match model outputs."
                    )
                else:
    ...
    if max_clips_to_load is not None:
        ...
        try:
            ...
            print(
                f"WARNING: train_test_split failed with stratify={stratify_arg}: {e}. Retrying without stratify."
            )
            X_train, X_test, y_train, y_test = train_test_split(
        ...
        print(
            f"INFO: Training clips: {X_train.shape[0]}, Test clips: {X_test.shape[0]}"
        )
        print(f"INFO: Class distribution in Training set (Top 5):")

        train_labels = (
            np.argmax(y_train, axis=1) if getattr(y_train, "ndim", 1) > 1 else y_train
        )
    ...
    if should_train:
        if model is None:
            ...
            model = build_hybrid_vfer_model(
                TIME_STEPS, IMG_HEIGHT, IMG_WIDTH, CHANNELS, NUM_CLASSES
            )
    ...
    if all_visualization_data:
        ...
        print(
            "WARNING: Could not generate visualization image (no valid clips loaded for visualization)."
        )
